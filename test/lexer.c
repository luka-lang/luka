...
extern char *lexer_lex_number(const char *source, size_t *index, t_logger *logger);
extern char *lexer_lex_identifier(const char *source, size_t *index);
extern char *lexer_lex_string(const char *source, size_t *index, t_logger *logger);
...
{
    utest_fixture->index = 0;
    ASSERT_STREQ("123", lexer_lex_number("123", &utest_fixture->index, utest_fixture->logger));
...
{
    utest_fixture->index = 0;
    ASSERT_STREQ("123.5", lexer_lex_number("123.5", &utest_fixture->index, utest_fixture->logger));
...
UTEST_F(lexer, lex_number_works_not_from_start)
{
    ...
    ASSERT_STREQ("123.5", lexer_lex_number("let a = 123.5;", &utest_fixture->index, utest_fixture->logger));
...
{
    utest_fixture->index = 0;
    ASSERT_STREQ("3.14", lexer_lex_number("3.14f", &utest_fixture->index, utest_fixture->logger));
    ASSERT_EQ((size_t)4, utest_fixture->index);
}
...
UTEST_F(lexer, lex_identifier_valid_identifiers)
{
    ...
    utest_fixture->index = 0;
    ASSERT_STREQ("ident2", lexer_lex_identifier("ident2", &utest_fixture->index));
    ASSERT_EQ((size_t)5, utest_fixture->index);

    utest_fixture->index = 0;
    ASSERT_STREQ("my_ident", lexer_lex_identifier("my_ident", &utest_fixture->index));
...
{
    utest_fixture->index = 0;
    ASSERT_STREQ("", lexer_lex_string("\"\"", &utest_fixture->index, utest_fixture->logger));
    ASSERT_EQ((size_t)0, utest_fixture->index);
}
